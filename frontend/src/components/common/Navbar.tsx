...
export function Navbar() {
  ...
  return (
    <>
      <aside className="fixed inset-y-0 left-0 z-50 hidden w-[280px] border-r border-white/[0.07] bg-[#111210]/97 p-5 backdrop-blur-2xl lg:flex lg:flex-col">
          ...
          {browseGroups.map(({ label, icon: Icon, type, links }) => {
            ...
            return (
              <details key={type} className="group/nav" open={active || undefined}>
                ...
                <div className="mb-2 ml-[1.35rem] mt-1 border-l border-white/[0.08] pl-4">
                  {links.map((link) => <Link key={link.label} to={link.to} className="block rounded-lg px-3 py-2 text-xs font-medium text-zinc-500 transition-colors hover:bg-white/[0.045] hover:text-zinc-200">{link.label}</Link>)}
                </div>
              </details>
            )
          })}
